...
class MultiNavigator(Navigator):
    ...
    DISTANCE_TOLERANCE = 1.00 # When the robot is within this distance of a waypoint, we publish a new goal (to make the path smoother)
    ...
    def wayposeNavigation(self, wayposes):
        if (not self.use_waypoints):
            (point, orientation, is_goal_point) = wayposes[-1]
            self.goToPose(point, orientation)
        else:
            for (point, orientation, is_goal_point) in wayposes:
                self.reserveWaypoint(point)
                #self.goToPose(point, orientation)
                if (is_goal_point): # If we want the turtlebot to finish going to the goal
                    self.goToPose(point, orientation)
                else:
    ...
    def releaseWaypoint(self, point):
        ...
        if (point != None):
            ...
            print "Waypoint server received release request, waited for " + str(time.time() - t)
    ...
    def fixOrientation(self, orientation):
        if (self.orientation_fixing):
            pass
